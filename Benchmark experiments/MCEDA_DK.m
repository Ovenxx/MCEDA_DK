...
function [nd_pop,nd_fitness,time]=MCEDA_DK(instances,vehicle_cap,vehicle_num,N,k,G,popN,Miu,ALpha,BEta)
tic
lambda=0.90;
customer_pos=instances(:,2:3);
customer_dem=instances(:,4:5);
customer_dem(:,1)=-customer_dem(:,1); 
customer_time=instances(:,6:8); 
scale_m=ceil(sqrt(N-1)); 
scale_delta=ceil(sqrt(N-1)/3); 
neisize=scale_m-scale_delta:scale_m+scale_delta;
D=distance_matrix(N,customer_pos);
% Initialization Stage
SC=proba_transform(D,N,lambda); 
TC=time_compatibility(customer_time,D,N); 
H=SC.*TC; 
[Population,Fitness]=construct_population(customer_time,D,customer_dem,vehicle_cap,vehicle_num,H,N,k*popN,Miu,ALpha,BEta); 
...
for g=1:G
    ...
    [new_Population,new_Fitness]=MCE(customer_time,D,customer_dem,vehicle_cap,vehicle_num,H,CM1,CM2,N,popN,Miu,ALpha,BEta);
    ...
    [Population_TFR,Fitness_TFR]=TFR(Population,Fitness,customer_pos,customer_dem,customer_time,vehicle_cap,D,popN,N,vehicle_num,Miu,ALpha,BEta);
    ...
    for n=1:size(nd_pop,1)
        VN(n)=sum(nd_pop(n,:)==1)-1;
    end
    dataVN(g).VN=VN;
end
time=toc;
...
function D=distance_matrix(N,customer_pos)
D=zeros(N,N);
for n1=1:N-1
    for n2=n1:N
        D(n1,n2)=norm(customer_pos(n1,:)-customer_pos(n2,:));
        D(n2,n1)=D(n1,n2);
    end
end
end
...
function [Population,Fitness]=MCE(customer_time,D,customer_dem,vehicle_cap,vehicle_num,H,CM1,CM2,N,popN,Miu,ALpha,BEta)
w=linspace(0,1,popN);
Population=zeros(popN,N+vehicle_num);
Fitness=zeros(popN,2);
for n=1:popN
    P=H.*(w(n)*CM1+(1-w(n))*CM2);
    [Population(n,:),Fitness(n,:)]=construct_population(customer_time,D,customer_dem,vehicle_cap,vehicle_num,P,N,1,Miu,ALpha,BEta);
end
end

function [Population,Fitness]=construct_population(customer_time,D,customer_dem,vehicle_cap,vehicle_num,P,N,popN,Miu,ALpha,BEta)
...
for n1=1:popN
    ...
    for n2=1:ns-1
        ...
        V_std=V*100/6;
        e_012=fi_e/V_std+fi_s*V_std^2+fi_w*(Wv+3*F_Loads);
        CBE=CBE+e_012*D(s(n2),s(n2+1))*100/6*rou; 
    end
...
function [Population,Fitness]=TFR(Population,Fitness,customer_pos,customer_dem,customer_time,vehicle_cap,D,popN,N,vehicle_num,Miu,ALpha,BEta)
dem_delivery=customer_dem(:,1); 
...
for n=1:popN
    ...
    if sum(Population(n,:)==1)>=3 
        [Population(n,:),Fitness(n,:)]=inter_TFR_exchange_SWAPstar(Population(n,:),Fitness(n,:),customer_pos,ne_dem_delivery,dem_pickup,customer_time,vehicle_cap,D,N,vehicle_num,gre,Miu,ALpha,BEta,max_fragment_length);
        [Population(n,:),Fitness(n,:)]=inter_TFR(Population(n,:),Fitness(n,:),customer_pos,ne_dem_delivery,dem_pickup,customer_time,vehicle_cap,D,N,vehicle_num,gre,Miu,ALpha,BEta,max_fragment_length);
    end
...
function [individual,fitness]=inter_TFR(individual,fitness,customer_pos,dem_delivery,dem_pickup,customer_time,vehicle_cap,D,N,vehicle_num,gre,Miu,ALpha,BEta,max_fragment_length)
s=individual;
...
xxx=tour;
xxx(start_p:end_p)=[];
cluster=zeros(number_of_depots,2);
cluster_d=zeros(1,number_of_depots);
for n=1:number_of_depots
    midd=s(index_of_depots(n)+1:index_of_depots(n+1)-1);
    if length(midd)~=1
        cluster(n,:)=mean(customer_pos(s(index_of_depots(n)+1:index_of_depots(n+1)-1),:));
    else
        cluster(n,:)=customer_pos(midd,:);
    end
end
for n=1:number_of_depots
    if n~=tour_index1
        cluster_d(n)=norm(cluster(tour_index1,:)-cluster(n,:));
    end
end
...
function [individual,fitness]=inter_TFR_exchange_SWAPstar(individual,fitness,customer_pos,dem_delivery,dem_pickup,customer_time,vehicle_cap,D,N,vehicle_num,gre,Miu,ALpha,BEta,max_fragment_length)
s=individual;
index_of_depots=find(s==1);
number_of_depots=length(index_of_depots)-1;
...
for n=1:number_of_depots
    if n~=tour_index
        cluster_d(n)=norm(cluster(tour_index,:)-cluster(n,:));
    end
end
cluster_d=max(cluster_d)-cluster_d+0.00001;
